...
void RawDecoderSpec::run(framework::ProcessingContext& ctx)
{
  ...
  constexpr uint16_t FEE_HCAL = 0xbeef; // TODO --> Change with whatever is needed for HCAL
    ...
    if (rawData.header != nullptr && rawData.payload != nullptr) {
      ...
      while (currentpos < databuffer.size()) {
        ...
        if (o2::raw::RDHUtils::getMemorySize(rdh) > o2::raw::RDHUtils::getHeaderSize(rdh)) {
          ...
          LOG(debug) << "Next RDH: ";
          LOG(debug) << "Found fee                   0x" << std::hex << fee << std::dec << " (System " << (fee == FEE_PADS ? "Pads" : (fee == FEE_HCAL ? "HCAL" : "Pixels")) << ")";
        ...
        if (trigger & o2::trigger::HB) {
          ...
          if (o2::raw::RDHUtils::getStop(rdh)) {
            ...
            if (rawbuffer.size()) {
              ...
              if (currentfee == FEE_PADS) { // Use FEE ID 0xcafe for PAD data
                // Pad data
                if (mUsePadData) {
              ...
              } else if (currentfee == FEE_HCAL) { // TODO: set to real HCAL FEE ID !!!!!! <-------------
                // Hcal data
                if (mUseHcalData) {
  ...
  auto foundHBFprtTFHcal = mNumHBFperTFHcal.find(numHBFHcalTF);
  if (foundHBFprtTFHcal != mNumHBFperTFHcal.end()) {
    foundHBFprtTFHcal->second++;
  } else {
    mNumHBFperTFHcal.insert(std::pair<int, int>{numHBFHcalTF, 1});
  }
...
void RawDecoderSpec::sendOutput(framework::ProcessingContext& ctx)
{
  ...
  ctx.outputs().snapshot(framework::Output{o2::header::gDataOriginFOC, "HCALPCBS", mOutputSubspec}, mOutputHcalPCBs);
...
int RawDecoderSpec::decodeHcalData(const gsl::span<const char> hcalWords, o2::InteractionRecord& hbIR)
{
  ...
  constexpr std::size_t EVENTSIZEHCALGBT = 1180,
                        EVENTSIZECHAR = EVENTSIZEHCALGBT * sizeof(HCALGBTWord) / sizeof(char);
...
{
  gsl::span<const HCALGBTWord> hcalWordsGBT(reinterpret_cast<const HCALGBTWord*>(hcalWords.data()), hcalWords.size() / sizeof(HCALGBTWord));
  mHcalDecoder.reset();
...
void RawDecoderSpec::buildEvents()
{
  ...
  for (const auto& [hbir, hbf] : mHBFs) {
    ...
    if (hbf.mPadEvents.size() != n ||
      ...
      //   continue;
      // }
      LOG(debug) << "HBF: " << hbf.mPixelTriggers.size() << " triggers, " << hbf.mPadEvents.size() << " pad events, " << hbf.mPixelEvent.size() << " pixel events, " << hbf.mHCALEvents.size() << " hcal events" << std::endl;
      for (std::size_t itrg = 0; itrg < hbf.mPixelTriggers.size(); itrg++) {
        auto startPads = mOutputPadLayers.size(),
             startHCAL = mOutputHcalPCBs.size(),
        ...
        mOutputTriggerRecords.emplace_back(hbf.mPixelTriggers[itrg], startPads, constants::PADS_NLAYERS, startHCAL, constants::HCAL_NPCBS, startChips, eventPixels.size(), startHits, eventHits.size());
      }
      ...
      for (std::size_t itrg = 0; itrg < hbf.mPixelTriggers.size(); itrg++) {
        ...
        std::copy(eventHits.begin(), eventHits.end(), std::back_inserter(mOutputPixelHits));
        std::copy(eventPixels.begin(), eventPixels.end(), std::back_inserter(mOutputPixelChips));
        mOutputTriggerRecords.emplace_back(hbf.mPixelTriggers[itrg], startPads, 0, startHCAL, constants::HCAL_NPCBS, startChips, eventPixels.size(), startHits, eventHits.size());
      }
      ...
      for (std::size_t itrg = 0; itrg < hbf.mPadEvents.size(); itrg++) {
        ...
        mOutputTriggerRecords.emplace_back(hbir, startPads, constants::PADS_NLAYERS, startHCAL, constants::HCAL_NPCBS, startChips, 0, startHits, 0);
      }
    } else if (mTimeframeHasHcalData) {
      // only hcal data available, set hcal pcbs and use IR of the HBF
